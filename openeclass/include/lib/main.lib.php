...
/*
 Debug MySQL queries
-------------------------------------------------------------------------
it is better to use the function below instead of the usual db_query()
...
-----------------------------------------------------------------------
*/
function db_query($sql, $newdb = FALSE) {
	global $db;
	if ($newdb) {
...
function db_query_input($query,$string_variables,$new_db) {
	global $db;
	if ($new_db) {
		mysqli_select_db($db,$new_db);
	}
	$result='';
	...
	if($string_variables!=''){
		...
		switch (func_num_args()) {
			case 4:
				$a=func_get_arg(3);
				mysqli_stmt_bind_param($statement,$string_variables,$a);

				break;
			case 5:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b);
				break;
			case 6:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				$c=func_get_arg(5);
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b,$c);
				break;
			case 7:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				$c=func_get_arg(5);
				$d=func_get_arg(6);
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b,$c,$d);
				break;
			case 8:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				...
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b,$c,$d,$e);
				break;
			case 9:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				...
				$e=func_get_arg(7);
				$f=func_get_arg(8);
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b,$c,$d,$e,$f);
				break;
			case 10:
				$a=func_get_arg(3);
				$b=func_get_arg(4);
				...
				$f=func_get_arg(8);
				$g=func_get_arg(9);
				mysqli_stmt_bind_param($statement,$string_variables,$a,$b,$c,$d,$e,$f,$g);
				break;
		}
...
// in an associative array. Compared to the PHP mysqli_fetch_array(),
// it proceeds in a single pass.
function db_fetch_all($sqlResultHandler, $resultType = MYSQL_ASSOC) {
	$rowList = array();

	while( $row = mysqli_fetch_array($sqlResultHandler, $resultType) )
	{
		$rowList [] = $row;
	}

	mysqli_free_result($sqlResultHandler);
...
function escapeSimple($str)
{
	...
	// 	{
	// 		return @mysqli_real_escape_string($db,$str, $db);
	// 	}
	// 	else
	// 	{
...
function uid_to_username($uid)
{
	global $mysqlMainDb;
	global $db;
	if ($r = mysqli_fetch_row(db_query(
	"SELECT username FROM user WHERE user_id = '".mysqli_real_escape_string($db,$uid)."'",
	$mysqlMainDb))) {
		return $r[0];
	} else {
		return FALSE;
...
function uid_to_name($uid)
{
	global $mysqlMainDb;
	global $db;
	if ($r = mysqli_fetch_row(db_query("SELECT CONCAT(nom, ' ', prenom)
		FROM user WHERE user_id = '".mysqli_real_escape_string($db,$uid)."'", $mysqlMainDb))) {
		return $r[0];
	} else {
		return FALSE;
	}
...
function uid_to_firstname($uid)
{
        global $mysqlMainDb;
		global $db;
        if ($r = mysqli_fetch_row(db_query("SELECT prenom
		FROM user WHERE user_id = '".mysqli_real_escape_string($db,$uid)."'", $mysqlMainDb))) {
                return $r[0];
        } else {
                return FALSE;
...
function uid_to_surname($uid)
{
        global $mysqlMainDb;
		global $db;
        if ($r = mysqli_fetch_row(db_query("SELECT nom
		FROM user WHERE user_id = '".mysqli_real_escape_string($db,$uid)."'", $mysqlMainDb))) {
                return $r[0];
        } else {
                return FALSE;
...
function uid_to_email($uid)
{
        global $mysqlMainDb;
		global $db;
        if ($r = mysqli_fetch_row(db_query("SELECT email
		FROM user WHERE user_id = '".mysqli_real_escape_string($db,$uid)."'", $mysqlMainDb))) {
                return $r[0];
        } else {
                return FALSE;
...
function uid_to_am($uid)
{
	...
	if ($r = mysqli_fetch_array(db_query("SELECT am from user
...
function gid_to_name($gid)
{
	global $currentCourseID;
	global $db;
	if ($r = mysqli_fetch_row(db_query("SELECT name FROM student_group
		WHERE id = '".mysqli_real_escape_string($db,$gid)."'", $currentCourseID))) {
                return $r[0];
	} else {
                return FALSE;
	}
...
function user_exists($login) {
  ...
  $username_check = db_query("SELECT username FROM `$mysqlMainDb`.user
	WHERE username='".mysqli_real_escape_string($db,$login)."'");
...
function find_faculty_by_name($name) {
	$code = mysqli_fetch_row(db_query("SELECT code FROM faculte
		WHERE name = '$name'"));
	if (!$code) {
		return FALSE;
	} else {
...
function new_code($fac) {
	global $mysqlMainDb;
	global $db;
	mysqli_select_db($db,$mysqlMainDb);
	$gencode = mysqli_fetch_row(db_query("SELECT code, generator
	...
	do {
		...
		db_query("UPDATE $mysqlMainDb.faculte SET generator = '$gencode[1]'
			WHERE id = '$fac'");
	} while (mysqli_select_db($code));
	mysqli_select_db($mysqlMainDb);
...
function last_login($uid)
{
        ...
        list($last_login) = mysqli_fetch_row($q);
        if (!$last_login) {
...
function make_clickable_path($dbTable, $path)
{
	...
	foreach (explode('/', $path) as $component) {
		if (empty($component)) {
			...
			$row = mysqli_fetch_array(db_query ("SELECT filename FROM $dbTable
...
function display_activation_link($module_id) {
	...
	$v = mysqli_fetch_array(db_query("SELECT lien FROM accueil
...
function visible_module($module_id) {
	...
	$v = mysqli_fetch_array(db_query("SELECT visible FROM accueil
		WHERE id ='$module_id'", $currentCourseID));
...
function move_order($table, $id_field, $id, $order_field, $direction, $condition = '')
{
        ...
        list($current) = mysqli_fetch_row($sql);
        ...
        if ($sql and mysqli_num_rows($sql) > 0) {
                list($next_id, $next) = mysqli_fetch_row($sql);
...
function add_units_navigation($entry_page = FALSE)
{
		...
		if (isset($_GET['unit'])) {
			...
			$unit_id = intval($_SESSION['unit']);
                ...
                if ($q and mysqli_num_rows($q) > 0) {
                        list($unit_name) = mysqli_fetch_row($q);
