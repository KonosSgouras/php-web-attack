...
if(count($_COOKIE)==0){
	header('Set-Cookie: PHPSESSID= ' . session_id() . '; Secure ; Path=/ ; SameSite=Strict; Max-Age=2592000 ;');
}



if(count($_COOKIE)==0){
    header('Set-Cookie: PHPSESSID= ' . session_id() . '; path=/; SameSite=Lax;');
}
...
if (!isset($webDir)) {
	...
	@include(htmlspecialchars(strip_tags($relPath)) . "config/config.php");
	if (!isset($webDir)) {
...
if (mysql_version()) db_query($db, "SET NAMES utf8");
mysqli_select_db($db, $mysqlMainDb);

// include_messages
include("${webDir}modules/lang/".htmlspecialchars(strip_tags($language))."/common.inc.php");
$extra_messages = "${webDir}/config/".htmlspecialchars(strip_tags($language)).".inc.php";
if (file_exists($extra_messages)) {
        include $extra_messages;
} else {
        $extra_messages = false;
}
include("${webDir}modules/lang/".htmlspecialchars(strip_tags($language))."/messages.inc.php");
if ($extra_messages) {
        include $extra_messages;
}
...
if (isset($require_current_course) and $require_current_course) {
	if (!isset($_SESSION['dbname'])) {
		...
		while ($theCourse = mysqli_fetch_array($result)) {
		...
		if (isset($uid)) {
        			...
        			$res2 = db_query_input("SELECT statut FROM cours_user
                                                  WHERE user_id = ? AND
                                                        cours_id = ?",'ii',FALSE,$uid,$cours_id);
        			if ($res2 and mysqli_num_rows($res2) > 0) {
	        			list($statut) = mysqli_fetch_row($res2);
		        	}
                        }
		}
...
if(isset($currentCourse) && file_exists($module_ini_dir = getcwd() . "/module.ini.php") && !$is_adminOfCourse && @$ignore_module_ini != true) {
	...
	$publicModules = array();
	while ($moduleIDs = mysqli_fetch_array($result)) {
