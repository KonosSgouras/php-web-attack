...
if ($is_adminOfCourse and isset($currentCourseID)) {
	if (isset($_GET['hide']) and $_GET['hide'] == 0) {
		db_query_input("UPDATE accueil SET visible = 0 WHERE id=?", 'i',$currentCourseID,$eclass_module_id);
	} else if (isset($_GET['hide']) and $_GET['hide'] == 1) {
		db_query_input("UPDATE accueil SET visible = 1 WHERE id=?", 'i',$currentCourseID,$eclass_module_id);
	}
}
...
include (htmlspecialchars(strip_tags($relPath)) . "template/template.inc.php");
include ('tools.php');
