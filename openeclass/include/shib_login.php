...
$r = mysqli_fetch_array(db_query("SELECT auth_settings FROM auth WHERE auth_id = 6"));
...
if (mysqli_num_rows($r) > 0) { // if shibboleth user found 
	while ($myrow = mysqli_fetch_array($r)) {
		...
		db_query_input("UPDATE user SET nom = ?, prenom = ?, email = ?
		WHERE username = ?",'ssss',FALSE,$shib_nom,$shib_prenom,$shib_email,$shib_uname);

		$r2 = db_query_input($sqlLogin,'s',FALSE,$shib_uname);
		while ($myrow2 = mysqli_fetch_array($r2)) {
			$uid = $myrow2["user_id"];
