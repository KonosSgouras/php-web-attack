...
function mysql_table_exists($db, $table)
{
	...
	return mysqli_num_rows($exists) == 1;
}
...
function add_index($index, $table, $column)  {
	...
	if ($num_of_args <= 3) {
		...
		while ($i = mysqli_fetch_array($ind_sql))  {
			if ($i['Key_name'] == $index) {
...
function accueil_tool_missing($define_var) {
        $r = db_query("SELECT id FROM accueil WHERE define_var = '$define_var'");
...
function upgrade_course_2_1_3($code, $extramessage = '')
{
	...
	while ($dbox = mysqli_fetch_array($d)) {
...
function upgrade_course_old($code, $lang, $extramessage = '')
{
	...
	while ($dbox = mysqli_fetch_array($d)) {
        ...
        $db_query_result = db_query($sql, $code);
        $event_counter=0;
        while ($myAgenda = mysqli_fetch_array($db_query_result)) {
        ...
        while ($group = mysqli_fetch_array($s)) {
	...
	while ($dbox = mysqli_fetch_array($d)) {
       ...
       db_query("DELETE FROM accueil WHERE (id = 12 OR id = 13 OR id = 11 OR id=6)", $code);
        ...
        // add full text indexes for search operation (ginetai xrhsh @db_query(...) giati ean
        ...
        @db_query("ALTER TABLE `document` ADD FULLTEXT `document` (`filename` ,`comment` ,`title`,`creator`,`subject`,`description`,`author`,`language`)");
...
function document_upgrade_file($path, $data)
{
        ...
        if (mysqli_num_rows($r) > 0) {
                $current_filename = mysqli_fetch_array($r);
...
function fix_document_date_and_format($code)
{
        ...
        while ($file = mysqli_fetch_array($q)) {
                $path = $base . $file['path'];
                if (!file_exists($path)) {
