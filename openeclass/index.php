...
if (isset($mysqlServer) and isset($mysqlUser) and isset($mysqlPassword)) {
	...
	//echo $mysqlServer," ",$mysqlUser," ",$mysqlPassword," ",$mysqlMainDb;

	$db = mysqli_connect($mysqlServer,$mysqlUser,$mysqlPassword,$mysqlMainDb);
...
//old
// if (isset($mysqlMainDb)) $selectResult = mysqli_select_db($db,$mysqlMainDb,$db);
...
if (isset($_SESSION['shib_uname'])) { // authenticate via shibboleth
	...
	if(!empty($submit)) {
		...
		$sqlLogin= "SELECT user_id, nom, username, password, prenom, statut, email, perso, lang FROM user WHERE username=?";
		//$result = mysql_query($sqlLogin);
		//$result = mysqli_execute_query($db,'SELECT user_id, nom, username, password, prenom, statut, email, perso, lang FROM user WHERE username=?', [$uname]);
		
		//new instructions
		// $statement = mysqli_prepare($db, $sqlLogin);
 		// mysqli_stmt_bind_param($statement, 's', $uname );
		// mysqli_stmt_execute($statement);
 		// $result = mysqli_stmt_get_result($statement);

		$result=db_query_input($sqlLogin,'s',FALSE,$uname);
		...
		if (!isset($uid)) {
			...
			// $statement = mysqli_prepare($db, );
 			// mysqli_stmt_bind_param($statement, 'is', $uid,$_SERVER[REMOTE_ADDR]);
			// mysqli_stmt_execute($statement);
			db_query_input("INSERT INTO loginout (loginout.idLog, loginout.id_user, loginout.ip, loginout.when, loginout.action) VALUES ('', '?', '?', NOW(), 'LOGIN')",'is',FALSE,$uid,$_SERVER[REMOTE_ADDR]);
		}
...
//../cources/TMA100/work/6445574c99d83/Marios%20Konstantinos%20
...
elseif ((isset($logout) && isset($uid)) OR (1==1)) {
	if (isset($logout) && isset($uid)) {
		db_query("INSERT INTO loginout (loginout.idLog, loginout.id_user,
