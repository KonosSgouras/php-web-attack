...
if (isset($_POST["submit"])) {
	...
	$errorExists = false;
        foreach ($selectCourse as $key => $value) {
                ...
                if ($course_info) {
                        $row = mysqli_fetch_array($course_info);
	...
	if (!$fac) {
		...
		if (isset($result))  {
            ...
            <tbody>";
			$k = 0;
			while ($fac = mysqli_fetch_array($result)) {
				if ($k%2==0) {
					$tool_content .= "\n        <tr>";
				} else {
					$tool_content .= "\n        <tr class='odd'>";
...
function getfacfromfc( $dep_id) {
	...
	$fac = mysqli_fetch_row(db_query("SELECT name FROM faculte WHERE id = '$dep_id'"));
	if (isset($fac[0]))
		return $fac[0];
	else
		return 0;
...
function getfcfromuid($uid) {
	$res = mysqli_fetch_row(db_query("SELECT department FROM user WHERE user_id = '$uid'"));
	if (isset($res[0]))
		return $res[0];
	else
		return 0;
...
function expanded_faculte($fac_name, $facid, $uid) {
	...
	if ($numoftypes > 1) {
		...
		$counter = 1;
		while ($typesArray = mysqli_fetch_array($typesresult)) {
			$t = $typesArray['type'];
...
function dep_selection($fc) {
	...
	while ($facs = mysqli_fetch_array($result)) {
...
{
	$res = mysqli_fetch_row(db_query("SELECT visible FROM cours WHERE cours_id = $cours_id"));
	if ($res[0] == 0) {
		return true;
	} else {
		return false;
