...
function get_auth_id()
{
  ...
  $auth_method = db_query($sql,$db);
  if($auth_method)
  {
		$authrow = mysqli_fetch_row($auth_method);
		if(mysqli_num_rows($auth_method)==1)
		{
	    $auth_id = $authrow[0];
	    return $auth_id;
		}
...
function get_auth_active_methods()
{
        ...
        if($auth_method) {
                $auth_methods = array();
                while($authrow = mysqli_fetch_row($auth_method)) {
...
{
	global $db;
	$qry = "SELECT * FROM auth WHERE auth_id = '".mysqli_real_escape_string($db,$auth)."'";
	$result = db_query($qry);
	if($result) {
		if(mysqli_num_rows($result)==1) {
			$auth_row = mysqli_fetch_array($result,MYSQL_ASSOC);
			return $auth_row;
		} else {
			return 0;
		}	
...
function auth_user_login ($auth, $test_username, $test_password)  {
	...
	case '1':
		...
		global $db;
	    $sql = "SELECT user_id FROM user WHERE username='".mysqli_real_escape_string($db,$test_username)."' 
		AND password='".mysqli_real_escape_string($db,$test_password)."'";
	    $result = db_query($sql);
	    if(mysqli_num_rows($result)==1) {
    		$testauth = true;
	    } else {
	...
	case '5':
	    ...
	    if($link) {
		$db_ext = mysqli_select_db($db,$dbname);
		if($db_ext) {
		    	$qry = "SELECT * FROM ".$dbname.".".$dbtable." 
				WHERE ".$dbfielduser."='".mysqli_real_escape_string($db,$test_username)."' 
				AND ".$dbfieldpass."='".mysqli_real_escape_string($db,$test_password)."'";
		    	$res = db_query($qry,$link);
		    	if($res) {
