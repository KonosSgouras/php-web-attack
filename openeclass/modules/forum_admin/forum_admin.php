...
if(isset($forumgo)) {
	...
	if ($result and mysqli_num_rows($result) > 0) {
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			$forum_moderator, $forum_type) = mysqli_fetch_row($result)) {
				if ($i%2==1) {
					$tool_content .= "\n<tr>";
				} else {
					$tool_content .= "\n<tr class=\"odd\">";
	...
	elseif(isset($forumgoedit)) {
		...
		$forum_type) = mysqli_fetch_row($result);
		$tool_content .= "
		...
		while(list($cat_id, $cat_title) = mysqli_fetch_row($result)) {
			if ($cat_id == $cat_id_1) {
	...
	elseif(isset($forumcatedit)) {
		...
		list($cat_id, $cat_title) = mysqli_fetch_row($result);
		$tool_content .= "
	...
	elseif(isset($forumgosave)) {
		...
		list($forum_moderator) = mysqli_fetch_row($result);
	...
	elseif(isset($forumgoadd)) {
		...
		while ($my_forum_id = mysqli_fetch_array($idforum)) {
			$forid=$my_forum_id[0];
		}
		// --------------------------------
		// notify users 
	...
	elseif(isset($forumcatdel)) {
		...
		while(list($forum_id) = mysqli_fetch_row($result)) {
	...
	elseif(isset($forumgodel)) {
		...
		if(isset($forumcatnotify)) { // modify forum category notification
			$rows = mysqli_num_rows(db_query("SELECT * FROM forum_notify 
		...
		while(list($cat_id, $cat_title) = mysqli_fetch_row($result)) {
			...
			$numbers = mysqli_fetch_array($gets);
			list($forum_cat_action_notify) = mysqli_fetch_row(db_query("SELECT notify_sent FROM forum_notify 
