...
if (isset($_REQUEST['userGroupId'])) {
        ...
        list($tutor_id, $forum_id) = mysqli_fetch_row(db_query("SELECT tutor, forumId FROM student_group WHERE id='$userGroupId'", $currentCourseID));
        ...
        if ($req and mysqli_num_rows($req) > 0) {
                list($userGroupId, $forum_id) = mysqli_fetch_row($req);
                $is_tutor = true;
        } else {
...
if ($forum_id === false) {
        list($forum_id) = mysqli_fetch_row(db_query("SELECT forumId FROM student_group WHERE id = " . user_group($uid)));
}
