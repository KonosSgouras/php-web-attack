...
$countRegistered = mysqli_num_rows(db_query("SELECT id FROM user_group 
	WHERE team='$userGroupId'", $currentCourse));
$total = mysqli_fetch_array(db_query("SELECT maxStudent FROM student_group 
...
if (isset($registration)) {
	if (($statut != 10) and ($countRegistered < $totalRegistered)) {
		$sqlExist=db_query("SELECT id FROM `$dbname`.user_group
			WHERE user='$uid' AND team='$userGroupId'");
		$countExist = mysqli_num_rows($sqlExist);
		if($countExist == 0) {
			$sqlReg=db_query("INSERT INTO `$dbname`.user_group (user, team)
...
$sqlGroup=db_query("SELECT secretDirectory FROM `$currentCourse`.student_group WHERE id='$userGroupId'");
while ($myGroup = mysqli_fetch_array($sqlGroup)) {
	$secretDirectory = $myGroup['secretDirectory'];
}

// name and description
mysqli_select_db($db,$dbname);
$resultGroup=db_query("SELECT name, description, tutor, forumId FROM student_group WHERE id='$userGroupId'");
while ($myGroup = mysqli_fetch_array($resultGroup))
{
	...
	$sqlTutor=db_query("SELECT tutor, user_id, nom, prenom, email, forumId
...
$resultMember=db_query("SELECT nom, prenom, email, am
...
if(($countMember==0)) {
	...
	while ($myMember = mysqli_fetch_array($resultMember)){
