...
list($tutor_id) = mysqli_fetch_row(db_query("SELECT tutor FROM student_group WHERE id='$userGroupId'", $currentCourseID));
...
if ($is_adminOfCourse or $is_tutor)  {
	if (isset($submit)) {
                $sender = mysqli_fetch_array(db_query("SELECT email, nom, prenom FROM user WHERE user_id = $uid", $mysqlMainDb));
                $sender_name = $sender['prenom'] . ' ' . $sender['nom'];
                $sender_email = $sender['email'];
                $emailsubject = $intitule." - ".$subject;
                $emailbody = "$body_mail\n\n$langSender: $sender[nom] $sender[prenom] <$sender[email]>\n$langProfLesson\n";

		$req = db_query("SELECT user FROM `$dbname`.user_group WHERE team = '$userGroupId'");
		while ($userid = mysqli_fetch_array($req)) {
                        $r = db_query("SELECT email FROM user where user_id='$userid[0]'", $mysqlMainDb);
			list($email) = mysqli_fetch_array($r);
