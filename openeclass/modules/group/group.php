...
mysqli_select_db($db,"$currentCourse");
...
if(isset($_REQUEST['creation']) && $is_adminOfCourse) {
	// Create a hidden category for group forums
        ...
        if ($req and mysqli_num_rows($req) > 0) {
                list($cat_id) = mysqli_fetch_row($req);
        } else {
	...
	for ($i = 1; $i <= $group_quantity; $i++) {
		...
		db_query("INSERT INTO student_group (maxStudent, secretDirectory)
			VALUES ('".mysqli_real_escape_string($db,$group_max)."', '$secretDirectory')");
		...
		$forumInsertId=mysqli_insert_id($db);
...
if(isset($_REQUEST['properties']) && $is_adminOfCourse)
{
	@db_query("UPDATE group_properties
		SET self_registration='".mysqli_real_escape_string($db,$self_registration)."', private='".mysqli_real_escape_string($db,$private)."',
		forum='".mysqli_real_escape_string($db,$forum)."', document='".mysqli_real_escape_string($db,$document)."' WHERE id=1", $currentCourse);
...
elseif (isset($_REQUEST['delete_one']) && $is_adminOfCourse)
{
	...
	while ($myDir = mysqli_fetch_array($sqlDir)) {
...
elseif (isset($_REQUEST['fill']) && $is_adminOfCourse) {
	...
	while (list($idGroup,$places) = mysqli_fetch_array($resGroups)) {
		$placeAvailableInGroups[$idGroup]= $places;
	}

	$resUsers = db_query($sqlUsers = "select user, team from user_group");
	while (list($idUser, $idGroup) = mysqli_fetch_array($resUsers)) {
	...
	while (isset($placeAvailableInGroups) and is_array($placeAvailableInGroups) and (!empty($placeAvailableInGroups)) and list($idUser) = mysqli_fetch_array($resUserSansGroupe))
	{
...
while ($myTutor = mysqli_fetch_array($sqlTutor)) {
...
if ($is_adminOfCourse) {
	...
	while ($myProperties = mysqli_fetch_array($resultProperties))
	{
		$tool_content .= "<tr><td>";
	...
	$myIterator=0;
	$num_of_groups = mysqli_num_rows($groupSelect);
	// groups list
	if ($num_of_groups > 0) {
		$tool_content .= "<br />
	...
	while ($group = mysqli_fetch_array($groupSelect)) {
		...
		$countRegistered = mysqli_num_rows($resultRegistered);
		if ($myIterator%2 == 0) {
			$tool_content .= "<tr>";
		} else {
	...
	$countUsers = mysqli_num_rows($coursUsersSelect);
...
// else student view
	...
	while ($mySelfReg = mysqli_fetch_array($sqlSelfReg)) {
	...
	while ($myTeamUser = mysqli_fetch_array($findTeamUser)) {
