...
if (isset($_POST['back1']) or !isset($_POST['visit'])) {
   ...
   // display form
	...
	<td>";
	list($homefac) = mysqli_fetch_row(db_query("SELECT department FROM user WHERE user_id=$uid"));
	$facs = db_query("SELECT id, name FROM faculte order by id");
	while ($n = mysqli_fetch_array($facs)) {
		$fac[$n['id']] = $n['name'];
	}
...
if (isset($_POST['create_course'])) {
        ...
        db_query("INSERT INTO cours SET
                        ...
                        faculteid = '$facid',
                        first_create = NOW()");
        $new_cours_id = mysqli_insert_id($db);
        ...
        db_query("INSERT INTO cours_faculte SET
                        faculte = '$faculte',
                        code = '$repertoire',
                        facid = '$facid'");
