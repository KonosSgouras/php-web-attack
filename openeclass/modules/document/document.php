...
$d = mysqli_fetch_row(db_query("SELECT doc_quota FROM cours WHERE code='$currentCourseID'"));
$diskQuotaDocument = $d[0];
mysqli_select_db($db,$currentCourseID);
...
if (@$action2=="download")
{
	...
	if (strpos($real_file, '/../') === FALSE) {
		...
		$result = db_query ("SELECT filename FROM document WHERE path LIKE '%$id%'");
...
{       // teacher only
	...
	if (is_uploaded_file(@$userFile)) {
		...
		if ($diskUsed + @$_FILES['userFile']['size'] > $diskQuotaDocument) {
			$dialogBox .= "<p class='caution_small'>$langNoSpace</p>";
                        ...
                        $result = db_query ("SELECT filename FROM document WHERE filename LIKE '%$uploadPath/$fileName%'");
                        $row = mysqli_fetch_array($result);
                                if (!empty($row['filename']))
                                {
                                ...
                                } else //to arxeio den vrethike sth vash ara mporoume na proxwrhsoume me to upload
                                {
                                        ...
                                        path	=	'".mysqli_real_escape_string($db,$uploadPath2)."',
                                        filename =	'".mysqli_real_escape_string($db,$fileName)."',
                                        visibility =	'v',
                                        comment	=	'".mysqli_real_escape_string($db,$file_comment)."',
                                        category =	'".mysqli_real_escape_string($db,$file_category)."',
                                        title =	'".mysqli_real_escape_string($db,$file_title)."',
                                        creator	=	'".mysqli_real_escape_string($db,$file_creator)."',
                                        date	= '".mysqli_real_escape_string($db,$file_date)."',
                                        date_modified	=	'".mysqli_real_escape_string($db,$file_date)."',
                                        subject	=	'".mysqli_real_escape_string($db,$file_subject)."',
                                        description =	'".mysqli_real_escape_string($db,$file_description)."',
                                        author	=	'".mysqli_real_escape_string($db,$file_author)."',
                                        format	=	'".mysqli_real_escape_string($db,$file_format)."',
                                        language =	'".mysqli_real_escape_string($db,$file_language)."',
                                        copyrighted	=	'".mysqli_real_escape_string($db,$file_copyrighted)."'";
	...
	**************************************/
	...
	if (isset($move))
	{
		...
		$result = db_query("SELECT * FROM $dbTable WHERE path=" . autoquote($move));
	...
	******************************************/
	...
	if (isset($rename))
	{
		...
		$result = db_query("SELECT * FROM $dbTable WHERE path='$rename'");
		$res = mysqli_fetch_array($result);
	...
	if (isset($mkVisibl) || isset($mkInvisibl))
	{
		...
		db_query ("UPDATE $dbTable SET visibility='".$newVisibilityStatus."' WHERE path = '$visibilityPath'");
