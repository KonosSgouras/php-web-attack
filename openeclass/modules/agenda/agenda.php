...
if ($is_adminOfCourse) {
	...
	if (isset($mkInvisibl) or isset($mkVisibl)) { 
		if (@$mkInvisibl == true) {
			...
			$p_sql= "DELETE FROM agenda WHERE lesson_code = '$currentCourseID' 
				AND lesson_event_id ='".mysqli_real_escape_string($db,$id)."'";
		} elseif (@$mkVisibl == true) {
			$sql = "UPDATE agenda SET visibility = 'v' WHERE id='".mysqli_real_escape_string($db,$id)."'";
			$p_sql = "SELECT id, titre, contenu, DAY, HOUR, lasting
				FROM agenda WHERE id='".mysqli_real_escape_string($db,$id)."'";
			$perso_result = db_query($p_sql, $currentCourseID);
			$perso_query_result = mysqli_fetch_row($perso_result);
	...
	if (isset($_POST['submit'])) {
		...
		if(isset($id) && $id) {
			$sql = "UPDATE agenda
                	SET titre='".mysqli_real_escape_string($db,trim($titre))."',
                	contenu='".mysqli_real_escape_string($db,trim($contenu))."',
                	day='".mysqli_real_escape_string($db,$date_selection)."',
                	hour='".mysqli_real_escape_string($db,$hour)."',
                	lasting='".mysqli_real_escape_string($db,$lasting)."'
                	WHERE id='".mysqli_real_escape_string($db,$id)."'";
			...
			$sql = "INSERT INTO agenda (id, titre, contenu, day, hour, lasting)
        		VALUES (NULL,'".mysqli_real_escape_string($db,trim($titre))."',
				'".mysqli_real_escape_string($db,trim($contenu))."', 
				'".mysqli_real_escape_string($db,$date_selection)."',
				'".mysqli_real_escape_string($db,$hour)."',
				'".mysqli_real_escape_string($db,$lasting)."')";
			unset($id);
			unset($contenu);
			unset($titre);
		}
