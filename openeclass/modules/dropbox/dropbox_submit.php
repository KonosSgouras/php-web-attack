...
if (!isset( $_POST['authors']) || !isset( $_POST['description']))
	...
	if (!$error) {
		...
		if (!$error) {
			...
			//new code xss prostasia	
				$description=htmlspecialchars(strip_tags($_POST['description']));
				$authors = htmlspecialchars(strip_tags($_POST['authors']));
				new Dropbox_SentWork($uid, $dropbox_title, $description, $authors, $dropbox_filename, $dropbox_filesize, $newWorkRecipients);
			}
		}
		chdir ($cwd);
	} //end if(!$error)
	...
	if (!preg_match($dropbox_cnf["mailingZipRegexp"], $mailing_title, $nameParts))
	{
		...
		if (!$error)
		{
			...
			if ( isset( $_GET['mailingSend']))
			{
				...
				$result = db_query($sql) or die($dropbox_lang["queryError"]);
			}
