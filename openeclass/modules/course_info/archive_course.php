...
function create_backup_file($file) {
	...
	list($ver) = mysqli_fetch_array(db_query("SELECT `value` FROM `$mysqlMainDb`.config WHERE `key`='version'"));
	fputs($f, "<?\n\$eclass_version = '$ver';\n\$version = 2;\n\$encoding = 'UTF-8';\n");
...
function backup_course_units($f) {
	...
	while($q = mysqli_fetch_array($res)) {
		...
		$begin = true;
		while($q2 = mysqli_fetch_array($res2)) {
			if ($begin) {
				$begin = !$begin;
				fputs($f, "\n");
			} else {
...
function backup_course_db($f, $course) {
	...
	while ($r = mysqli_fetch_row($res_tables)) {
		...
		$res_create = mysqli_fetch_array(db_query("SHOW CREATE TABLE $tablename"));
		$schema = $res_create[1];
		...
		if ($tablename == 'user_group') {
			...
			if (mysqli_num_rows($res) > 0) {
				...
				$counter = 1;
				while($rowdata = mysqli_fetch_row($res)) {
					if (($counter % 30) == 1) {
						if ($counter > 1) {
							fputs($f, "\n\");\n");
						}
