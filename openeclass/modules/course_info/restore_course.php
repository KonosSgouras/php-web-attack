...
if (isset($send_archive) and $_FILES['archiveZipped']['size'] > 0) {
	...
	ob_start();
	include("".htmlspecialchars(strip_tags($restoreThis))."/backup.php");
	if ($encoding != 'UTF-8') {
...
elseif (isset($_POST['pathOf4path'])) {
	...
	$action = 0;
	ob_start();
	include(htmlspecialchars(strip_tags($_POST['restoreThis'] )). '/backup.php');
	$tool_content .= ob_get_contents();
	ob_end_clean();
} else {
	...
	if (mysqli_num_rows($u) > 0) 	{
		...
		db_query("INSERT into `$mysqlMainDb`.user
			...
			join(", ", array(
				...
				")");
		$userid_map[$userid] = mysqli_insert_id($db);
	}
...
function query($sql) {
	...
	if (!$action) return;
	global $db;
	mysqli_select_db($db,$new_course_code);
	if ($encoding != 'UTF-8') {
...
function dropbox_person($fileId, $personId) {
	...
	mysqli_select_db($db,$course_code);
...
function create_course($code, $lang, $title, $desc, $fac, $vis, $prof, $type) {
	...
	if (mysqli_select_db($db,$repertoire)) {
