...
if($is_adminOfCourse) {
	...
	$sql="SELECT DISTINCT uid FROM `exercise_user_record` WHERE eid='".mysqli_real_escape_string($db,$_GET['exerciseId'])."'";
	$result = db_query($sql);
	while($row=mysqli_fetch_array($result)) {
		$sid = $row['uid'];
		$StudentName = db_query("select nom, prenom from user where user_id='$sid'", $mysqlMainDb);
		$theStudent = mysqli_fetch_array($StudentName);	
		$nom = $theStudent["nom"];
		...
		$sql2="SELECT DATE_FORMAT(RecordStartDate, '%Y-%m-%d / %H:%i') AS RecordStartDate, 
			...
			FROM `exercise_user_record` WHERE uid='$sid' AND eid='".mysqli_real_escape_string($db,$_GET['exerciseId'])."'";
