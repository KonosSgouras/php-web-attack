...
function get_syslang_string($sys_lang, $string) {
	include('language/lang_'.htmlspecialchars(strip_tags($sys_lang)).'.php');
	$ret_string = $$string;
	return($ret_string);
}
...
function forum_category($id) {
	...
	if ($r = mysqli_fetch_row(db_query("SELECT cat_id FROM forums WHERE forum_id=$id", $currentCourseID))) {
		return $r[0];
	} else {
		return FALSE;
	}
...
function category_name($id) {
	...
	if ($r = mysqli_fetch_row(db_query("SELECT cat_title FROM catagories WHERE cat_id=$id", $currentCourseID))) {
		return $r[0];
	} else {
		return FALSE;
	}
