...
if ($is_adminOfCourse) { // course admin 
	if (isset($submit) && $submit) {
		...
		$row1 = mysqli_fetch_row(db_query("SELECT forum_name FROM forums WHERE forum_id='$forum_id'"));
		$forum_name = $row1[0];
		$row2 = mysqli_fetch_row(db_query("SELECT topic_title FROM topics WHERE topic_id='$topic_id'"));
		$topic_title = $row2[0];
