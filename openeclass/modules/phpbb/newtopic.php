...
//new code xss
	$$ubject = htmlspecialchars(strip_tags($subject));
	...
	$sql = "INSERT INTO topics (topic_title, topic_poster, forum_id, topic_time, topic_notify, nom, prenom)
			VALUES (?, '$uid', ?, '$time', 1, '$nom', '$prenom')";
	$result = db_query_input($sql, 'si',$currentCourseID,$subject,$forum);
	...
	if (!$result = db_query($sql, $currentCourseID)) {
		...
		if ($post_id) {
			$sql = "INSERT INTO posts_text (post_id, post_text)
					VALUES ($post_id, ?)";
			$result = db_query_input($sql, 's',$currentCourseID,$message);
			$sql = "UPDATE topics
	...
	$sql = "UPDATE forums
		...
		WHERE forum_id = ?";
	$result = db_query_input($sql,'i', $currentCourseID,$forum);
	...
	$sql = db_query("SELECT DISTINCT user_id FROM forum_notify 
			WHERE (forum_id = ? OR cat_id = $category_id) 
			AND notify_sent = 1 AND course_id = $cours_id",'i', $mysqlMainDb,$forum);
