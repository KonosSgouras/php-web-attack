...
if (isset($post_id) && $post_id) {
	...
	$sql .= "WHERE (f.forum_id = ?) AND (t.topic_id = $topic)";
	...
	$sql .= "WHERE (f.forum_id = ?) AND (t.topic_id = $topic) AND (t.forum_id = f.forum_id)";	
}
...
if (isset($submit) && $submit) {
	...
	$sql = "INSERT INTO posts (topic_id, forum_id, poster_id, post_time, poster_ip, nom, prenom)
			VALUES ('$topic', ?, '$uid','$time', '$poster_ip', '$nom', '$prenom')";
	$result = db_query_input($sql, 'i',$currentCourseID,$forum);
	$this_post = mysqli_insert_id($db);
	if ($this_post) {
