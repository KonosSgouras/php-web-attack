...
if ($total_categories) {
	...
	$tool_content .= "<tbody>";
	while ($forum_data = mysqli_fetch_array($f_res)) {
	...
	for($i=0; $i < $total_categories; $i++) {
		...
		list($action_notify) = mysqli_fetch_row(db_query("SELECT notify_sent FROM forum_notify 
		...
		for ($x=0; $x < count($forum_row); $x++) {
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				list($forum_action_notify) = mysqli_fetch_row(db_query("SELECT notify_sent FROM forum_notify 
