...
function getUserDocuments($param = null, $type)
{
	...
	for ($i=0;$i<$max_repeat_val;$i++) { //each iteration refers to one lesson
		...
		$db_query_result = db_query($docs_query_new[$i], $lesson_code[$i]);
		if ($num_rows = mysqli_num_rows($db_query_result) > 0) {
			$getNewDocs = true;
		...
		while ($myDocuments = mysqli_fetch_row($db_query_result)) {
			if ($myDocuments){
	...
	if ($getNewDocs) {
		...
		for ($i=0; $i < $max_repeat_val; $i++){
			$db_query_result = db_query($docs_query_memo[$i], $lesson_code[$i]);
			if (mysqli_num_rows($db_query_result) > 0) {
				$docsLessonData = array();
				$docsData = array();
				array_push($docsLessonData, $lesson_title[$i]);
				array_push($docsLessonData, $lesson_code[$i]);
