...
function getUserForumPosts($param, $type)
{
	...
	for ($i=0;$i<$max_repeat_val;$i++) {
		...
		$db_query_result = db_query($forum_query_new, $lesson_code[$i]);
		...
		while ($myForumPosts = mysqli_fetch_row($db_query_result)) {
			if ($myForumPosts){
	...
	if ($getNewPosts) {
		...
		for ($i=0; $i < $max_repeat_val; $i++){
			$db_query_result = db_query($forum_query_memo, $lesson_code[$i]);
