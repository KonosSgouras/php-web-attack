...
hContent;
    ...
    list($announcementNumber) = mysqli_fetch_row($result);
    ...
    if (isset($thisAnnouncementId) && $thisAnnouncementId && isset($sortDirection) && $sortDirection) {
        ...
        while (list ($announcementId, $announcementOrder) = mysqli_fetch_row($result)) {
...
// DISPLAY ANNOUNCEMENT CONTENT
	$tool_content .= "\n      </tr>";
            $iterator ++;
            $k++;
        } // end while ($myrow = mysqli_fetch_array($result))
        $tool_content .= "
      </tbody>
      </table>";
