...
switch( $cmd )
{
    ...
    case "exRename" :
        	...
        	&& is_numeric($_POST['module_id']) )
        {
            ...
            $sql="SELECT `name`
                  FROM `".$TABLEMODULE."`
                  WHERE `name` = '". mysqli_real_escape_string($db,$_POST['newName'])."'
            ...
            if($num == 0 ) // "name" doesn't already exist
            {
                ...
                $query="UPDATE `".$TABLEMODULE."`
                        SET `name`= '". mysqli_real_escape_string($db,$_POST['newName'])."'
    ...
    case "exComment":
        	...
        	&& isset($_POST['module_id']) && is_numeric($_POST['module_id']) )
        {
            $sql = "UPDATE `".$TABLEMODULE."`
                    SET `comment` = \"". mysqli_real_escape_string($db,$_POST['comment']) ."\"
                    WHERE `module_id` = '". (int)$_POST['module_id']."'";
            db_query($sql);
        }
        break;
...
$num_results = mysqli_num_rows($query_num_results);
