			...
			case "create" :
				...
				if( isset($_POST["newPathName"]) && $_POST["newPathName"] != "") {
					...
					$sql = "SELECT `name` FROM `".$TABLELEARNPATH."`
						WHERE `name` = '". mysqli_real_escape_string($db,$_POST['newPathName']) ."'";
					$query = db_query($sql);
					...
					if($num == 0) { // "name" doesn't already exist
						...
						$sql = "INSERT INTO `".$TABLELEARNPATH."` (`name`, `comment`, `rank`)
							VALUES ('". mysqli_real_escape_string($db,$_POST['newPathName']) ."','" . mysqli_real_escape_string($db,trim($_POST['newComment']))."',".(int)$order.")";
						$lp_id = db_query($sql);
					} else {
