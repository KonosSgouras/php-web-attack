    ...
    if (isset($_REQUEST['submitInsertedDocument']) && isset($_POST['insertDocument_'.$iterator]) )
    {
        ...
        if ( check_name_exist($sourceDoc) ) // source file exists ?
        {
            ...
            if($num == 0)
            {
                ...
                $query = db_query($sql);
                $insertedModule_id = mysqli_insert_id($db);
                ...
                $query = db_query($sql);
                $insertedAsset_id = mysqli_insert_id($db);
...
$d = mysqli_fetch_array(db_query("SELECT filename FROM $TABLEDOCUMENT WHERE path='$curDirPath'"));
