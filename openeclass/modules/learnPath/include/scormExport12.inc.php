...
class ScormExport
{
    ...
    function fetch()
    {
        ...
        $list = mysqli_fetch_array($result, MYSQL_ASSOC);
        if ( empty($list) )
        {
            $this->error[] = $langLearningPathNotFound;
            return false;
        ...
        while ($module = mysqli_fetch_array($result, MYSQL_ASSOC))
        {
...
<frameset border="0" rows="100%,*" onload="immediateComplete()">
    ...
    <frame src="SCOFunctions.js">
        ...
        function createDescFrameFile($fileName)
        {
            ...
            $course_description = "";
            //mysqli_select_db($db,"$currentCourseID",$db);
			...
			if (mysqli_num_rows($res) >0 )
			{
				$course_description .= "
					<hr noshade size=\"1\">";
				while ($bloc = mysqli_fetch_array($res))
				{ 
					$course_description .= "
					<H4>
						".$bloc["title"]."
