...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
        ...
        if (!$allow_username_change) {
			//$username_form = strip_tags($username_form);
                $username_form = $uname;
        }
	// check if username exists
	$username_check=db_query_input("SELECT username FROM user WHERE username='?",'s',FALSE,$username_form);
	while ($myusername = mysqli_fetch_array($username_check))
	{
...
// NEW CODE
		...
		if(db_query_input("UPDATE user
	        SET nom=?, prenom=?,
	        username=?, email=?, am=?,

	            perso='$persoStatus', lang='$langcode'
	        WHERE user_id='".$_SESSION["uid"]."'",'ssssi',FALSE,$nom_form,$prenom_form,$username_form,$email_form,$am_form)) {
...
$result=db_query($sqlGetInfoUser);
