...
$nameTools = $langVideo;
$tool_content = "";
$d = mysqli_fetch_array(db_query("SELECT video_quota FROM cours WHERE code='$currentCourseID'",$mysqlMainDb));
...
if (isset($_POST['submit']) or isset($_POST['edit_submit'])) {
	if($id) {
		$sql = "UPDATE $table SET url='".mysqli_real_escape_string($db,$url)."', titre='".mysqli_real_escape_string($db,$titre)."', description='".mysqli_real_escape_string($db,$description)."',creator='".mysqli_real_escape_string($db,$creator)."',publisher='".mysqli_real_escape_string($db,$publisher)."'
			WHERE id='".mysqli_real_escape_string($db,$id)."'";
	} else {
		if(isset($URL)) {
			if ($titre == "") $titre = $URL;
			$url = $URL;
			$sql = "INSERT INTO videolinks (url,titre,description,creator,publisher,date) VALUES ('$url','".mysqli_real_escape_string($db,$titre)."','".mysqli_real_escape_string($db,$description)."','".mysqli_real_escape_string($db,$creator)."','".mysqli_real_escape_string($db,$publisher)."','".mysqli_real_escape_string($db,$date)."')";
		} else {
			...
			if (($file_name != "") && ($file_size <= $diskQuotaVideo )) {
				...
				$path = "/".$safe_filename;
				$url="$file_name";
				$sql = "INSERT INTO video (path, url, titre, description, creator, publisher, date) VALUES ('$path', '$url','".mysqli_real_escape_string($db,$titre)."','".mysqli_real_escape_string($db,$description)."','".mysqli_real_escape_string($db,$creator)."','".mysqli_real_escape_string($db,$publisher)."','".mysqli_real_escape_string($db,$date)."')";
			}
		}	// else
	...
	elseif (isset($delete)) {
		$sql_select="SELECT * FROM $table WHERE id='".mysqli_real_escape_string($db,$id)."'";
		$result = db_query($sql_select,$currentCourseID);
		$myrow = mysqli_fetch_array($result);
		if($table=="video") {
			unlink("$webDir/video/$currentCourseID/".$myrow['path']);
		}
		$sql = "DELETE FROM $table WHERE id='".mysqli_real_escape_string($db,$id)."'";
		$result = db_query($sql,$currentCourseID);
...
if (isset($id)) {
   if($id != "") {
	  $sql = "SELECT * FROM $table_edit WHERE id='".mysqli_real_escape_string($db,$id)."' ORDER BY titre";
      	  $result = db_query($sql,$currentCourseID);
      	  $myrow = mysqli_fetch_array($result);
          $id = $myrow[0];
...
<form method=\"POST\" action=\"$_SERVER[PHP_SELF]?submit=yes\" enctype=\"multipart/form-data\" id=\"edit_form\" onsubmit=\"return checkrequired(this, 'titre');\">
  ...
  <div id=\"operations_container\">
	...
	$count_video = mysqli_fetch_array(db_query("SELECT count(*) FROM video ORDER BY titre",$currentCourseID));
	$count_video_links = mysqli_fetch_array(db_query("SELECT count(*) FROM videolinks
