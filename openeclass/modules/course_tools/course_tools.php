...
if ($is_adminOfCourse){
	...
	if  (isset($_REQUEST['toolStatus']) ){
		...
		$prevAgendaStateRow = mysqli_fetch_row($res);
		...
		if (isset($tool_stat_active) && is_array($tool_stat_active)) {
			if (in_array(1, $tool_stat_active)) {
				...
				if ($prevAgendaStateRow[0] != 1) {
					...
					$db_query_result = db_query($sql, $currentCourseID);
	...
	if(isset($submit) &&  @$action == 2) {
		...
		while ($maxID = mysqli_fetch_row($res)) {
			$mID = $maxID[0];
		}
		...
		db_query("INSERT INTO accueil VALUES ($mID,
					$name_link,
					$link,
					'external_link',
					'1',
