        ...
        if (!$c) {
                if ($u == 1) {
                        ...
                        if ($total>0) {
                                ...
                                if ($q and mysqli_affected_rows($db) > 0) {
                                        $t = 2;
                                } else {
                                        $t = 3;
                                }
                        ...
                        if($u!=1) {
                                db_query("DELETE from admin WHERE idUser = '".mysqli_real_escape_string($db,$u)."'");
                        }
...
function get_uid_statut($u)
{
	global $mysqlMainDb;
        global $db;
	if ($r = mysqli_fetch_row(db_query("SELECT statut FROM user WHERE user_id = '".mysqli_real_escape_string($db,$u)."'",	$mysqlMainDb)))
	{
		return $r[0];
	}
	else
