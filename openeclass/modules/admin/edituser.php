...
if((!empty($u)) && ctype_digit($u) )	// validate the user id
{
	...
	{
		$sql = db_query("SELECT nom, prenom, username, password, email, phone, department, registered_at, expires_at, statut, am FROM user WHERE user_id = '$u'");
  ...
  <tr>
    <td colspan='2'>&nbsp;</td>
		...
		// check if username is free
  		$username_check=db_query("SELECT username FROM user WHERE username='".escapeSimple($username)."'");
		...
		if($registered_at>$expires_at) {
			...
			$qry = db_query($sql);
                        if (!$qry) {
                                ...
                                $num_update = mysqli_affected_rows($db);
                                if ($num_update == 1) {
