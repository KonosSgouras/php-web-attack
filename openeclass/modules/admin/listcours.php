...
// Manage list limits
$countcourses = mysqli_fetch_array(db_query("SELECT COUNT(*) AS cnt FROM cours"));
...
if (isset($search) && $search=="yes") {
	...
	if(!empty($searchtitle)) {
		$searchcours[] = "intitule LIKE '%".mysqli_escape_string($searchtitle)."%'";
	}
	if(!empty($searchcode)) {
		$searchcours[] = "code LIKE '%".mysqli_escape_string($searchcode)."%'";
	}
	if ($searchtype != "-1") {
		$searchcours[] = "visible = '".mysqli_escape_string($searchtype)."'";
	}
	if($searchfaculte != "0") {
		$searchcours[] = "faculte = '".mysqli_escape_string($searchfaculte)."'";
	}
	$query=join(' AND ',$searchcours);
	if (!empty($query)) {
		$sql=db_query("SELECT faculte, code, intitule, titulaires, visible, cours_id FROM cours 
