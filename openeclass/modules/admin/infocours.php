...
if (isset($submit))  {
  ...
  // Update query
	$sql = db_query("UPDATE cours SET faculte='$facname', titulaires='$titulaires', intitule='$intitule', faculteid='$facid' WHERE code='".mysqli_real_escape_string($db,$_GET['c'])."'");
	// Some changes happened
	if (mysqli_affected_rows($db) > 0) {
		$sql = db_query("UPDATE cours_faculte SET faculte='$facname', facid='$facid' WHERE code='".mysqli_real_escape_string($db,$_GET['c'])."'");
...
// Display edit form for course basic information
	...
	$row = mysqli_fetch_array(db_query("SELECT * FROM cours WHERE code='".mysqli_real_escape_string($db,$_GET['c'])."'"));
  ...
  // Construct select object for facultes
	...
	while ($myfac = mysqli_fetch_array($resultFac)) {
