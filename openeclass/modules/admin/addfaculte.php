...
if (!isset($a)) {
	...
	$a=mysqli_fetch_array(db_query("SELECT COUNT(*) FROM faculte"));
	// Construct a table
...
elseif ($a == 1)  {
	if (isset($add)) {
		...
		elseif (mysqli_num_rows(db_query("SELECT * from faculte WHERE code=" . autoquote($codefaculte))) > 0) {
			$tool_content .= "<p>".$langFCodeExists."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[PHP_SELF]?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check if faculty name already exists
		elseif (mysqli_num_rows(db_query("SELECT * from faculte WHERE name=" . autoquote($faculte))) > 0) {
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		elseif (mysqli_num_rows(db_query("SELECT * from faculte WHERE id <> $c AND name=" .
                                                   autoquote($faculte))) > 0) {
			$tool_content .= "<p>".$langFaculteExists."</p><br>";
			$tool_content .= "<p align='right'><a href='$_SERVER[PHP_SELF]?a=3&amp;c=$c'>$langReturnToEditFaculte</a></p>";
		} else {
