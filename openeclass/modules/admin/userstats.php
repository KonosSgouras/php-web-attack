...
{
	$u = (int)$u;
	$sql = db_query("SELECT nom, prenom, username, password, email, phone, department, registered_at, expires_at FROM user WHERE user_id = '$u'");
...
$i=0;
while ($leRecord = mysqli_fetch_array($leResultat)) {
	$when = $leRecord["when"];
