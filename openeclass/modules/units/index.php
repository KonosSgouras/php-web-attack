...
foreach (array('previous', 'next') as $i) {
        ...
        if ($q and mysqli_num_rows($q) > 0) {
                list($q_id, $q_title) = mysqli_fetch_row($q);
...
function check_admin_unit_resource($resource_id)
{
	...
	if ($is_adminOfCourse) {
		...
		if (mysqli_num_rows($q) > 0) {
			list($unit_id) = mysqli_fetch_row($q);
			return $unit_id;
		}
	}
	return false;
...
function show_resources($unit_id)
{
	...
	if (mysqli_num_rows($req) > 0) {
		list($max_resource_id) = mysqli_fetch_row(db_query("SELECT id FROM unit_resources
...
function show_doc($title, $comments, $resource_id, $file_id)
{
        ...
        if (mysqli_num_rows($r) == 0) {
                ...
                $file = mysqli_fetch_array($r, MYSQL_ASSOC);
...
function show_lp($title, $comments, $resource_id, $lp_id)
{
	...
	$r = db_query("SELECT * FROM lp_learnPath WHERE learnPath_id = $lp_id",
                      $currentCourseID);
	if (mysqli_num_rows($r) == 0) { // check if lp was deleted
		if (!$is_adminOfCourse) {
			return '';
		} else {
			$status = 'del';
			...
			$link = "<span class='invisible'>$title ($langWasDeleted)</span>";
                ...
                $lp = mysqli_fetch_array($r, MYSQL_ASSOC);
...
function show_work($title, $comments, $resource_id, $work_id, $visibility)
{
	...
	$r = db_query("SELECT * FROM assignments WHERE id = $work_id",
                      $currentCourseID);
	if (mysqli_num_rows($r) == 0) { // check if it was deleted
		if (!$is_adminOfCourse) {
			return '';
		} else {
			$status = 'del';
			...
			$exlink = "<span class='invisible'>$title ($langWasDeleted)</span>";
                ...
                $work = mysqli_fetch_array($r, MYSQL_ASSOC);
	...
	if (mysqli_num_rows($r) == 0) { // check if it was deleted
		if (!$is_adminOfCourse) {
			...
			$exlink = "<span class='invisible'>$title ($langWasDeleted)</span>";
                ...
                $exercise = mysqli_fetch_array($r, MYSQL_ASSOC);
...
function show_forum($type, $title, $comments, $resource_id, $ft_id, $visibility)
{
	...
	if ($type == 'forum') {
		...
		list($forum_id) = mysqli_fetch_array($r);
	...
	if (mysqli_num_rows($r) == 0) { // check if it was deleted
		if (!$is_adminOfCourse) {
			...
			$exlink = "<span class='invisible'>$title ($langWasDeleted)</span>";
                ...
                $wiki = mysqli_fetch_array($r, MYSQL_ASSOC);
