...
{
	list($order) = mysqli_fetch_array(db_query("SELECT MAX(`order`) FROM unit_resources WHERE unit_id=$id"));
...
function insert_video($id)
{
	...
	foreach ($_POST['video'] as $video_id) {
		...
		$row = mysqli_fetch_array(db_query("SELECT * FROM $table
...
function insert_work($id)
{
	...
	foreach ($_POST['work'] as $work_id) {
		$order++;
		$work = mysqli_fetch_array(db_query("SELECT * FROM assignments
...
function insert_exercise($id)
{
	...
	foreach ($_POST['exercise'] as $exercise_id) {
		$order++;
		$exercise = mysqli_fetch_array(db_query("SELECT * FROM exercices
...
function insert_forum($id)
{
	...
	foreach ($_POST['forum'] as $for_id) {
		...
		if (count($ids) == 2) {
			...
			$topic = mysqli_fetch_array(db_query("SELECT * FROM topics
			...
			$GLOBALS['mysqlMainDb']);		
                        ...
                        $forum_id = $ids[0];
			$forum = mysqli_fetch_array(db_query("SELECT * FROM forums
...
function insert_wiki($id)
{
	...
	foreach ($_POST['wiki'] as $wiki_id) {
		$order++;
		$wiki = mysqli_fetch_array(db_query("SELECT * FROM wiki_properties
