...
{
	$poll = mysqli_fetch_array(db_query("SELECT * FROM poll WHERE pid=$pid"));
	...
	$qnumber = 0;
	while ($theQuestion = mysqli_fetch_array($questions)) {
		...
		if ($qtype == 1) {
			...
			while ($theAnswer = mysqli_fetch_array($answers)) {
	...
	foreach ($questions as $i => $QuestionText) {
		...
		if (!empty($QuestionText)) {
			...
			db_query("INSERT INTO poll_question (pid, question_text, qtype) VALUES ('".
				...
				mysqli_real_escape_string($db,$QuestionText) . "', '$qtype')");
			$pqid = mysqli_insert_id($db);
			if ($question_types[$i] == 1) {
				...
				foreach ($answers as $j => $AnswerText) {
					...
					if (!empty($AnswerText)) {
						db_query("INSERT INTO poll_question_answer (pqid, answer_text)
							VALUES ($pqid, '".mysqli_real_escape_string($db,$AnswerText) ."')");
					}
				}
			}
		}
...
function createPoll($questions, $question_types) {
	...
	mysqli_select_db($db,$GLOBALS['currentCourseID']);
