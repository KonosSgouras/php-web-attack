...
if(!isset($_GET['sid']) || !is_numeric($_GET['sid'])) die();
	...
	$theSurvey = mysqli_fetch_array($current_survey);
...
if ($_GET['type'] == 2) { //TF
	...
	while ($theAnswer = mysqli_fetch_array($answers)) {
		...
		$theCreator = mysqli_fetch_array($answer_creator);
		...
		$qas = db_query("
			...
			while ($theQAs = mysqli_fetch_array($qas)) {	
...
// Get data to print pie chart ////////////////////////////////////////////////////
	...
	while ($theAnswer = mysqli_fetch_array($answers)) {
		...
		while ($theArid = mysqli_fetch_array($arids)) {
			...
			while ($theQ_Ts = mysqli_fetch_array($q_ts)) {
				if (!count($q_t_GD)) {
...
******************************************************************************/
   		...
   		$q_as = db_query("
			...
			$q_a_GD = array();
			while ($theQ_As = mysqli_fetch_array($q_as)) {
