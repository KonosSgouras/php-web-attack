	...
	foreach (array_keys($_POST) as $key) {
		...
		if ($counter == 2) {
			...
			$result1 = db_query("INSERT INTO survey VALUES ('".
				...
				mysqli_real_escape_string($db,$SurveyName) 	. "','".
				mysqli_real_escape_string($db,$CreationDate) . "','".
				mysqli_real_escape_string($db,$StartDate) 		. "','".
				mysqli_real_escape_string($db,$EndDate) 			. "','".
				mysqli_real_escape_string($db,$SurveyType) 	. "','".
				mysqli_real_escape_string($db,$SurveyActive) ."')");
				...
				if (substr($key, 0, 8) == "question") { //question
					...
					if ($$key != '') {
						...
						$result5 = db_query("INSERT INTO survey_question_answer VALUES ('0','".
							$sqid. "','".
							mysqli_real_escape_string($db,$AnwserText) ."')");
					}
				}
			}
	}
