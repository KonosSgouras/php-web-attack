...
$theSurvey = mysqli_fetch_array($survey);
...
function submitSurvey() {
	...
	mysqli_real_escape_string($db,$aid) . "','".
	mysqli_real_escape_string($db,$creator_id) . "','".
		...
		foreach (array_keys($_POST) as $key) {
			...
			if (($counter_foreach >= 4)&&
				...
				$result2 = db_query("INSERT INTO survey_answer_record VALUES ('0','".
					mysqli_real_escape_string($db,$aid). "','".
					mysqli_real_escape_string($db,$QuestionText). "','".
					mysqli_real_escape_string($db,$QuestionAnswer) ."')");
				}
			}  
