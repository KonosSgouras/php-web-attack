...
function printPollForm() {
	...
	$poll = db_query("SELECT * FROM poll WHERE pid='".mysqli_real_escape_string($db,$pid)."' "
		."ORDER BY pid", $currentCourse);
	$thePoll = mysqli_fetch_array($poll);
...
function submitPoll() {
	...
	//	$answer_text = htmlspecialchars(strip_tags($answer_text));
