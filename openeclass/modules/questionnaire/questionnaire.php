...
/*
		case 'sactivate':
		$sql = "UPDATE survey SET active='1' WHERE sid='".mysqli_real_escape_string($db,$_GET['sid'])."'";
		$result = db_query($sql,$currentCourseID);
		$GLOBALS["tool_content"] .= $GLOBALS["langSurveyActivated"];
			break;
		case 'sdeactivate':
			$sql = "UPDATE survey SET active='0' WHERE sid='".mysqli_real_escape_string($db,$_GET['sid'])."'";
				$result = db_query($sql,$currentCourseID);
...
// activate / dectivate polls
		case 'activate':
			$sql = "UPDATE poll SET active='1' WHERE pid='".mysqli_real_escape_string($db,$_GET['pid'])."'";
			$result = db_query($sql,$currentCourseID);
			$GLOBALS["tool_content"] .= "".$GLOBALS["langPollActivated"]."<br>";
			break;
		case 'deactivate':
			$sql = "UPDATE poll SET active='0' WHERE pid='".mysqli_real_escape_string($db,$_GET['pid'])."'";
			$result = db_query($sql, $currentCourseID);
...
if (isset($sdelete) and $sdelete='yes') {
				db_query("DELETE FROM survey WHERE sid=".mysqli_real_escape_string($db,$_GET['sid']));
        $sd = mysqli_fetch_array(db_query("SELECT aid FROM survey_answer
                                        WHERE sid=".mysqli_real_escape_string($db,$_GET['sid'])));
        db_query("DELETE FROM survey_answer_record WHERE aid='$sd[aid]'");
        db_query("DELETE FROM survey_answer WHERE sid=".mysqli_real_escape_string($db,$_GET['sid']));
        $sd = mysqli_fetch_array(db_query("SELECT sqid FROM survey_question
                                        WHERE sid=".mysqli_real_escape_string($db,$_GET['sid'])));
        db_query("DELETE FROM survey_question_answer WHERE sqid='$sd[sqid]'");
        db_query("DELETE FROM survey_question WHERE sid=".mysqli_real_escape_string($db,$_GET['sid']));
...
cData;
			...
			while ($theSurvey = mysqli_fetch_array($active_surveys)) {
				...
				if (($visibility)||($is_adminOfCourse)) {
					...
					$theCreator = mysqli_fetch_array($survey_creator);

					$sid = $theSurvey["sid"];
					$answers = db_query("SELECT * FROM survey_answer WHERE sid='$sid'", $currentCourse);
					$countAnswers = mysqli_num_rows($answers);
					...
					if ($is_adminOfCourse)   {
						...
						<img src='../../template/classic/img/".$visibility_gif.".gif' border='0'></a>  ".
							...
							$has_participated = mysqli_fetch_array(db_query("SELECT COUNT(*) FROM survey_answer
...
cData;
	...
	$index_aa = 1;
	$k =0;
		while ($thepoll = mysqli_fetch_array($active_polls)) {
		...
		if (($visibility) or ($is_adminOfCourse)) {
			...
			$has_participated = mysqli_fetch_array(db_query("SELECT COUNT(*) FROM poll_answer_record
